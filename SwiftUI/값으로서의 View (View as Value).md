...
1. **과거 보존 (diff의 전제)** — **가변** class 뷰는 변경이 복사 없이 **원본에 곧바로
   가해져** 이전 값이 소멸한다 → 비교 대상이 없어 diff 불가.
   struct는 이전 뷰 트리가 독립된 값으로 공짜로 살아남는다.
   막는 것은 class 자체가 아니라 **변경 가능한 공유 참조**다 — 아래 트레이드오프 표의
   불변 class(Flutter·React)는 같은 목적을 다른 길로 달성한다
